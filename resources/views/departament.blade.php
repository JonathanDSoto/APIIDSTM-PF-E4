...
@section('aditional_scripts')
    ...
    <script>
        const nombre_clave = document.getElementById('nombre_clave');
        const nombre = document.getElementById('nombre');
        const imagen = document.getElementById('dropzone-basic');

        const label = document.getElementById('exampleModalLabel1');
        ...
        function addNewEdificio() {
            label.textContent = "Agregar Departamento";
            btnAddDept.textContent = "Agregar Departament";
            nombre.value = '';
            nombre_clave.value = '';
            imagen.dropzone.removeAllFiles();
            ...
            btnAddDept.onclick = async () => {
                try {
                    ...
                    form.append('code_name', nombre_clave.value);

                    if (imagen.dropzone.files.length > 0) {
                        const file = imagen.dropzone.files[0];
        ...
        async function modifyDept(e) {
            label.textContent = "Editar Departamento";
            btnAddDept.textContent = "Editar Departamento";

            let buildingCard = e.target.closest('building-card');
            const buildingId = buildingCard.data.id;

            nombre.value = buildingCard.data.name;
            nombre_clave.value = buildingCard.data.codeName;
            ...
            btnAddDept.onclick = async () => {
                try {
                    ...
                    if (imagen && imagen.dropzone && imagen.dropzone.files.length > 0) {
                        const file = imagen.dropzone.files[0];
                        form.append('image', file, file.name);
                    }
                    ...
                    if (!response.ok) {
                        throw new Error(
                        `Error al actualizar el edificio. Código de estado: ${response.status}`);
                    }
