...
@section('aditional_scripts')
    ...
    <script>
        async function getRoles() {
            ...
            let response = await fetch(`${window.location.origin}/api/roles`, {
                method: 'GET',
                headers: new Headers({
            ...
            let selectRoles = document.getElementById('user-role');
            let data = await response.json();

            data.forEach(role => {
                selectRoles.innerHTML += `<option value="${role.id}">${role.name}</option>`
            });

            document.querySelector(".add-new").addEventListener('click', addUser)
        ...
        function addUser() {
            ...
            modal.show();
            document.querySelector(".add-new-user").reset();
            ...
            document.querySelector(".data-submit").onclick = async () => {
                ...
                if (isValid != "Invalid") {
                    ...
                    if (response.status == 200) {
                        Swal.fire({
                            title: "¡Muy bien! Usuario registrado satisfactoriamente",
                        ...
                        <form class="add-new-user pt-0" id="addNewUserForm" onsubmit="return false">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="add-user-name"
                                    placeholder="Ingresa su nombre" name="userName" aria-label="John Doe" />
                            </div>
                            <div class="mb-3">
                                <label class="form-label" for="add-user-lastname">Apellidos</label>
                                <input type="text" class="form-control" id="add-user-lastname"
                                    placeholder="Ingresa sus apellidos" name="userLastName" aria-label="John Doe" />
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" id="add-user-password" class="form-control"
                                    placeholder="Ingrese su contraseña" aria-label="Password" name="userPassword"
                                    required pattern="(?=.*[A-Z]).{6,}"
                                    title="Debe contener al menos 6 caracteres, incluyendo una mayúscula." />
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" id="add-user-password" class="form-control"
                                    placeholder="Ingrese su contraseña" aria-label="Password" name="userConfirmPassword"
                                    required pattern="(?=.*[A-Z]).{6,}"
                                    title="Debe contener al menos 6 caracteres, incluyendo una mayúscula." />
                                <div class="form-text">Debe contener al menos 6 caracteres, incluyendo una mayúscula.</div>
                            </div>
                            ...
                            <button type="reset" class="btn btn-label-secondary"
                                data-bs-dismiss="offcanvas">Reiniciar</button>
                        </form>
                    </div>
                </div>
            </div>
